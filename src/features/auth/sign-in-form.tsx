...
export function SignInForm({ initialError }: SignInFormProps) {
  ...
  return <section className="lm-panel w-full max-w-md p-6 sm:p-8">
    ...
    <div className="my-6 flex items-center gap-3 text-xs text-[var(--lm-muted)]"><span className="h-px flex-1 bg-[var(--lm-line)]" />أو<span className="h-px flex-1 bg-[var(--lm-line)]" /></div><button className="lm-button lm-button-quiet w-full gap-2" disabled={isLoading} onClick={signInWithGoogle} type="button"><span className="grid h-5 w-5 place-items-center rounded-full bg-[var(--lm-accent-soft)] text-xs text-[var(--lm-accent-dark)]">G</span>المتابعة باستخدام Google</button><Link className="lm-link mt-5 block text-center text-sm" href="/">تخطّي تسجيل الدخول والمتابعة لاحقًا</Link>{error ? <p className="mt-4 text-sm font-medium text-red-700" role="alert">{error}</p> : null}
  </section>;
}
